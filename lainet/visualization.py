...
def visualize_palette(label,palette=None):
    ...
    ax.set_yticks([i for i in range(label.size)])
...
def plot_cm(cm,title=None,normalize=False, labels=None, figsize=(10,8), path=None):
    ...
    if labels is None:
        fig = sns.heatmap(df_cm, annot=True, fmt='g', annot_kws={"size": 16}) # font size
    else:
        fig = sns.heatmap(df_cm, xticklabels=labels, fmt='g', yticklabels=labels,
...
def plot_chm(sample_id, msp_df,output_basename,output_file,palette=sns.color_palette(LAI_PALETTE).as_hex(), rm_img=False, img_name="chm_img"):
    ...
    #subprocess.run(["tagore","--i",tagore_df_fname,"-p",img_name,"--build","hg37","-f"])
